...
function BuyerRequest() {
  ...
  const fetchBuyerRequests = async (userId) => {
    try {
      setLoading(true);
      const response = await fetch(`${BASE_URL}/api/buyer-requests/${userId}`);
  ...
  const renderRequestItem = ({ item }) => (
    <TouchableOpacity
      ...
      onPress={() => navigation.navigate('BuyerRequestDetail', { requestId: item.requestId })}
    >
  ...
  return (
    <View style={styles.container}>
      <View style={styles.header}>
        <Text style={styles.headerTitle}>My Buyer Requests</Text>
        <Text style={styles.headerSubtitle}>Manage your posted buyer requests</Text>
      </View>

      <TouchableOpacity
        style={styles.createButton}
        onPress={() => navigation.navigate('CreateBuyerRequest')}
      >
      ...
      {buyerRequests.length === 0 ? (
        <View style={styles.emptyContainer}>
          ...
          <Text style={styles.emptyText}>No Buyer Requests</Text>
